...
CREATE TABLE rent_info(id SERIAL PRIMARY KEY ,
                       check_in DATE,
                       check_out DATE,
                       agency_id INT REFERENCES addresses(id),
...
CREATE TABLE houses(id SERIAL PRIMARY KEY,
                    house_type House_type,price INT,
                    rating INT,
                    description VARCHAR,
                    room INT,
                    furniture BOOLEAN,
...
ALTER TABLE agencies ADD CONSTRAINT c CHECK(phone_number LIKE '+996');
ALTER TABLE agencies DROP CONSTRAINT c;
ALTER TABLE owners ADD CONSTRAINT cu UNIQUE (email);
ALTER TABLE customers RENAME COLUMN familyStatus TO marital_status;
...
SELECT first_name,length(first_name)AS countOwner FROM owners join houses h on owners.id = h.owner_id ORDER BY first_name LIMIT 1;
-- 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
SELECT price from houses where price BETWEEN '1500' AND '2000'=true or false;
...
SELECT h.house_type,o.first_name,c.first_name,a.name FROM rent_info r JOIN houses h ON r.house_id = h.id
                                                                      JOIN owners o ON r.owner_id = o.id JOIN customers c  ON r.customer_id = c.id
                                                                      JOIN agencies a ON r.agency_id = a.id;
-- 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.
SELECT * FROM customers WHERE date_of_birth >'1997-07-19';
-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
SELECT h.house_type,h.rating ,o.first_name as owner_name FROM houses h join owners o ON h.owner_id = o.id  ORDER BY h.rating ASC ;
--7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
SELECT COUNT(*) AS apartment_count, SUM(price) AS total_price FROM houses WHERE house_type = 'apartment';
...
SELECT h.house_type,a.name FROM houses h join agencies a ON a.address_id=h.id  WHERE a.name='My_House';
